...
function targetnearestmob(_range)
	_range = _range or 35
	local function mobstats(_arg1,spot)

		local lvl, adjlvl, X, Z, Y, id, hostile, name, targetingID
		--name = memoryReadUStringPtr(proc,_arg1 + 0x34,0x0) or "LOL"
		name = "LOL"
		hostile = memoryReadInt(proc, _arg1 + 0x60) or 1
		lvl = memoryReadIntPtr(proc, _arg1 + 0x140,0x84) or 99
		targetingID = memoryReadInt( proc, _arg1 + 0x44) or 5000
		hp = memoryReadFloatPtr(proc, _arg1 + 0x168,0x8) or 500
		maxhp = memoryReadFloatPtr(proc, _arg1 + 0x168,0xC) or 500
	...
	for i = 0, size-1 do
		...
		if two ~= 0 then
			coordsupdate()
			count = count +1
			lvl, X, Z, Y, hostile, name, targetingID, hp, maxhp = mobstats(two,i*4)
			if X ~= nil then 
				--dist = 20
				dist = distance(player.X,player.Z,player.Y,X,Z,Y)
				if (hostile == 2 or hostile == 1) and lvl ~= 1 then
					table.insert(mobs,{spot = two,lvl = lvl, X=X, Z=Z, Y=Y, hostile=hostile,tarID = targetingID, dist = dist, HP = hp, MaxHP = maxhp, baseaddress = two,})
				end
			end
		end
	end
...
function followcharname(_name)
	local function playerstats2(_arg1)
		name = memoryReadUStringPtr(proc, addresses.objectArray,{0x28,_arg1,0x34,0x0})
		X = memoryReadFloatPtr(proc, addresses.objectArray,{0x28,_arg1,0x8,0x44,0x1C,0x5C,0xB4})
		Z = memoryReadFloatPtr(proc, addresses.objectArray,{0x28,_arg1,0x8,0x44,0x1C,0x5C,0xB8})
		Y = memoryReadFloatPtr(proc, addresses.objectArray,{0x28,_arg1,0x8,0x44,0x1C,0x5C,0xBC})
		return X, Z, Y, name
	end
	npc = {}
   local proc = getProc()
   size = memoryReadRepeat("intptr", proc, addresses.objectArray, 0x2C)
   print("The size of the array is "..size)
   for i = 1, size-1 do
      if memoryReadRepeat("intptr", proc, addresses.objectArray,{0x28,i*4}) ~= 0 then
         if memoryReadIntPtr(proc, addresses.objectArray,{0x28,i*4,0x8}) ~= 0 then
...
function mob()
   local proc = getProc()
   size = memoryReadRepeat("intptr", proc, addresses.objectArray, 0x1C)
   print("The size of the array is "..size)
   for i = 1, size-1 do
      if memoryReadRepeat("intptr", proc, addresses.objectArray,{0x14,i*4}) ~= 0 then
         if memoryReadRepeat("intptr", proc, addresses.objectArray,{0x14,i*4,0xC}) ~= 0 then
            print(i)
         end
      end
   end
